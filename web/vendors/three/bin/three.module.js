    ...
    function updateBuffer(buffer, attribute, bufferType) {
        ...
        if (updateRanges.length === 0) {
            ...
            for (let i = 0, l = updateRanges.length; i < l; i++) {
                ...
                gl.bufferSubData(
                    bufferType, range.start * array.BYTES_PER_ELEMENT,
...
function WebGLCapabilities(gl, extensions, parameters, utils) {
    ...
    function textureTypeReadable(textureType) {
        ...
        const halfFloatSupportedByExt = (textureType === HalfFloatType) && (extensions.has('EXT_color_buffer_half_float') || extensions.has(
            ...
            if (isEquirectMap || isCubeMap) {
                ...
                if (texture.isRenderTargetTexture && texture.pmremVersion !== currentPMREMVersion) {
                    ...
                    if (renderTarget !== undefined) {
                        ...
                        if ((isEquirectMap && image && image.height > 0) || (isCubeMap && image && isCubeTextureComplete(
                            image))) {
    ...
    function getExtension(name) {
        ...
        switch (name) {
            ...
            case 'WEBGL_depth_texture':
                extension = gl.getExtension('WEBGL_depth_texture') || gl.getExtension('MOZ_WEBGL_depth_texture') || gl.getExtension(
                    'WEBKIT_WEBGL_depth_texture')
                break

            case 'EXT_texture_filter_anisotropic':
                extension = gl.getExtension('EXT_texture_filter_anisotropic') || gl.getExtension(
                    'MOZ_EXT_texture_filter_anisotropic') || gl.getExtension('WEBKIT_EXT_texture_filter_anisotropic')
                break

            case 'WEBGL_compressed_texture_s3tc':
                extension = gl.getExtension('WEBGL_compressed_texture_s3tc') || gl.getExtension(
                    'MOZ_WEBGL_compressed_texture_s3tc') || gl.getExtension('WEBKIT_WEBGL_compressed_texture_s3tc')
                break

            case 'WEBGL_compressed_texture_pvrtc':
                extension = gl.getExtension('WEBGL_compressed_texture_pvrtc') || gl.getExtension(
...
function WebGLPrograms(renderer, cubemaps, cubeuvmaps, extensions, capabilities, bindingStates, clipping) {
    ...
    function getParameters(material, lights, shadows, scene, object) {
        ...
        const envMapCubeUVHeight = (!!envMap) && (envMap.mapping === CubeUVReflectionMapping) ?
            envMap.image.height :
            null
        ...
        const parameters = {
            ...
            decodeVideoTexture: HAS_MAP && (material.map.isVideoTexture === true) && (ColorManagement.getTransfer(
                material.map.colorSpace) === SRGBTransfer),
            decodeVideoTextureEmissive: HAS_EMISSIVEMAP && (material.emissiveMap.isVideoTexture === true) && (ColorManagement.getTransfer(
                material.emissiveMap.colorSpace) === SRGBTransfer),
            ...
            extensionClipCullDistance: HAS_EXTENSIONS && material.extensions.clipCullDistance === true && extensions.has(
                'WEBGL_clip_cull_distance'),
            extensionMultiDraw: (HAS_EXTENSIONS && material.extensions.multiDraw === true || IS_BATCHEDMESH) && extensions.has(
                'WEBGL_multi_draw'),
...
function WebGLShadowMap(renderer, objects, capabilities) {
    ...
    function renderObject(object, camera, shadowCamera, light, type) {
        ...
        if (visible && (object.isMesh || object.isLine || object.isPoints)) {
            ...
            if ((object.castShadow || (object.receiveShadow && type === VSMShadowMap)) && (!object.frustumCulled || _frustum.intersectsObject(
                object))) {
...
function WebGLTextures(_gl, extensions, state, properties, capabilities, utils, info) {
    ...
    const multisampledRTTExt = extensions.has('WEBGL_multisampled_render_to_texture') ?
        ...
        null
    const supportsInvalidateFramebuffer = typeof navigator === 'undefined' ?
        false :
    ...
    function getInternalFormat(internalFormatName, glFormat, glType, colorSpace, forceLinearTransfer = false) {
        ...
        if (glFormat === _gl.RGBA) {
            ...
            if (glType === _gl.UNSIGNED_BYTE) internalFormat = (transfer === SRGBTransfer) ?
                _gl.SRGB8_ALPHA8 :
                _gl.RGBA8
        ...
        if (renderTarget.isWebGLCubeRenderTarget) {
            ...
            for (let i = 0; i < 6; i++) {
                ...
                if (Array.isArray(renderTargetProperties.__webglFramebuffer[i])) {
                    ...
                    for (let level = 0; level < renderTargetProperties.__webglFramebuffer[i].length; level++) _gl.deleteFramebuffer(
            ...
            if (Array.isArray(renderTargetProperties.__webglFramebuffer)) {
                ...
                for (let level = 0; level < renderTargetProperties.__webglFramebuffer.length; level++) _gl.deleteFramebuffer(
            ...
            if (renderTargetProperties.__webglColorRenderbuffer) {
                ...
                for (let i = 0; i < renderTargetProperties.__webglColorRenderbuffer.length; i++) {
                    ...
                    if (renderTargetProperties.__webglColorRenderbuffer[i]) _gl.deleteRenderbuffer(
    ...
    function uploadTexture(textureProperties, texture, slot) {
        ...
        if (source.version !== sourceProperties.__version || forceUpload === true) {
            ...
            const unpackConversion = texture.colorSpace === NoColorSpace || workingPrimaries === texturePrimaries ?
                _gl.NONE :
    ...
    function convert(p, colorSpace = NoColorSpace) {
        ...
        if (p === RGB_ETC1_Format || p === RGB_ETC2_Format || p === RGBA_ETC2_EAC_Format) {
            ...
            if (extension !== null) {
                ...
                if (p === RGB_ETC1_Format || p === RGB_ETC2_Format) return (transfer === SRGBTransfer) ?
                    extension.COMPRESSED_SRGB8_ETC2 :
                    extension.COMPRESSED_RGB8_ETC2
                if (p === RGBA_ETC2_EAC_Format) return (transfer === SRGBTransfer) ?
    ...
    constructor(parameters = {}) {
        ...
        this.renderBufferDirect = function (camera, scene, geometry, material, object, group) {
            ...
            if (object.isBatchedMesh) {
                ...
                const maxInstanceCount = geometry._maxInstanceCount !== undefined ?
        ...
        this.render = function (scene, camera) {
            ...
            if (camera.isArrayCamera) {
                ...
                if (transmissiveObjects.length > 0) renderTransmissionPass(
                    opaqueObjects,
                    transmissiveObjects,
                    scene,
                    camera,
            ...
            if (renderStateStack.length > 0) {
                ...
                if (_clippingEnabled === true) clipping.setGlobalState(
        ...
        function renderTransmissionPass(opaqueObjects, transmissiveObjects, scene, camera) {
            ...
            if (currentRenderState.state.transmissionRenderTarget[camera.id] === undefined) {
                ...
                currentRenderState.state.transmissionRenderTarget[camera.id] = new WebGLRenderTarget(1, 1, {
                    ...
                    type: (extensions.has('EXT_color_buffer_half_float') || extensions.has('EXT_color_buffer_float')) ?
                        HalfFloatType :
                        UnsignedByteType,
        ...
        function getProgram(material, scene, object) {
            ...
            materialProperties.envMapRotation = (materialProperties.environment !== null && material.envMap === null) ?
        ...
        function setProgram(camera, scene, geometry, material, object) {
            ...
            const fog = scene.fog
            const environment = material.isMeshStandardMaterial ? scene.environment : null
            const colorSpace = (_currentRenderTarget === null) ?
        ...
        this.readRenderTargetPixels = function (renderTarget, x, y, width, height, buffer, activeCubeFaceIndex) {
            ...
            if (framebuffer) {
                ...
                try {
                    ...
                    const framebuffer = (_currentRenderTarget !== null) ?
